            ...
            <div class="flex items-center border-b pb-2 mb-2" v-for="item in cart.items">
                ...
                <div class="w-1/12">
                    <div class="inline-flex">
                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight font-mono text-right focus:outline-none focus:shadow-outline" type="number" min="1" v-model="item.qty">
                        <button @click="changeQty(item)" class="btn btn-primary ml-1" title="Update">
                ...
                <div class="w-1/12 flex justify-end text-right font-mono">
                    {{ item.total | price }}
                    <a href="#" @click.prevent="remove(item)" class="ml-2" title="Remove">
