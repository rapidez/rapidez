...
@section('content')
    <h1 class="font-bold text-4xl">@lang('Cart')</h1>
    ...
    <cart v-cloak>
        <div v-if="hasItems" slot-scope="{ cart, hasItems, changeQty, remove }">
            <div class="flex flex-wrap items-center border-b pb-2 mb-2" v-for="(item, productId, index) in cart.items">
                ...
                <div class="w-2/6 sm:w-1/6 lg:w-1/12 text-right font-mono pr-5">
                    @{{ item.price | price }}
                </div>
                ...
                <div class="w-2/6 sm:w-1/6 lg:w-1/12 flex justify-end text-right font-mono">
                    @{{ item.total | price }}
